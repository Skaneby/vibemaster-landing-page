...
const translations = {
  ...
  en: {
    ...
    "faq.q5":             "How does Vibemaster generate its tech plans?",
    "faq.a5":             "Vibemaster's tech plans are generated by <a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>, Google's large language model. Gemini is trained on a broad corpus of public data — documentation, developer forums, open-source repositories, and technical articles — and uses that knowledge to recommend tools, architectures, and patterns. Vibemaster does not independently select or rank the tools in your plan. Instead, our proprietary prompt architecture structures the questions we ask Gemini, which ensures the output is consistent and buildable. The AI tools that appear in your plan (such as Lovable, Bolt.new, or Cursor) are those that Gemini identifies as leading options based on its training data — we additionally verify that these tools work well with our generated output before surfacing them to users.",
  ...
  sv: {
    ...
    "faq.q5":             "Hur genererar Vibemaster sina tech-planer?",
    "faq.a5":             "Vibemasters tech-planer genereras av <a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>, Googles stora språkmodell. Gemini är tränad på ett brett korpus av offentlig data — dokumentation, utvecklarforum, open source-repositories och tekniska artiklar — och använder denna kunskap för att rekommendera verktyg, arkitekturer och mönster. Vibemaster väljer eller rangordnar inte själv verktygen i din plan. Istället strukturerar vår proprietära promptarkitektur de frågor vi ställer till Gemini, vilket säkerställer att resultatet är konsekvent och byggbart. De AI-verktyg som visas i din plan (som Lovable, Bolt.new eller Cursor) är de som Gemini identifierar som ledande alternativ baserat på sin träningsdata — vi verifierar dessutom att dessa verktyg fungerar väl med vårt genererade resultat innan vi visar dem för användare.",
  ...
  de: {
    ...
    "faq.q5":             "Wie generiert Vibemaster seine Tech-Pläne?",
    "faq.a5":             "Vibemasters Tech-Pläne werden von <a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>, Googles großem Sprachmodell, generiert. Gemini ist auf einem breiten Korpus öffentlicher Daten trainiert — Dokumentationen, Entwicklerforen, Open-Source-Repositories und technische Artikel — und nutzt dieses Wissen, um Tools, Architekturen und Muster zu empfehlen. Vibemaster wählt oder bewertet die Tools in deinem Plan nicht selbst. Stattdessen strukturiert unsere proprietäre Prompt-Architektur die Fragen, die wir Gemini stellen, was sicherstellt, dass der Output konsistent und baubar ist. Die KI-Tools in deinem Plan (wie Lovable, Bolt.new oder Cursor) sind jene, die Gemini basierend auf seinen Trainingsdaten als führende Optionen identifiziert — wir verifizieren zusätzlich, dass diese Tools gut mit unserem generierten Output funktionieren.",
  ...
  es: {
    ...
    "faq.q5":             "¿Cómo genera Vibemaster sus planes técnicos?",
    "faq.a5":             "Los planes técnicos de Vibemaster son generados por <a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>, el gran modelo de lenguaje de Google. Gemini está entrenado en un amplio corpus de datos públicos — documentación, foros de desarrolladores, repositorios de código abierto y artículos técnicos — y utiliza ese conocimiento para recomendar herramientas, arquitecturas y patrones. Vibemaster no selecciona ni clasifica independientemente las herramientas de tu plan. En su lugar, nuestra arquitectura de prompts propietaria estructura las preguntas que hacemos a Gemini, lo que garantiza que el resultado sea consistente y construible. Las herramientas de IA en tu plan (como Lovable, Bolt.new o Cursor) son aquellas que Gemini identifica como opciones líderes según sus datos de entrenamiento.",
  ...
  fr: {
    ...
    "faq.q5":             "Comment Vibemaster génère-t-il ses plans techniques?",
    "faq.a5":             "Les plans techniques de Vibemaster sont générés par <a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>, le grand modèle de langage de Google. Gemini est entraîné sur un large corpus de données publiques — documentation, forums de développeurs, dépôts open source et articles techniques — et utilise ces connaissances pour recommander des outils, des architectures et des modèles. Vibemaster ne sélectionne ni ne classe pas indépendamment les outils de ton plan. Au lieu de cela, notre architecture de prompts propriétaire structure les questions que nous posons à Gemini, ce qui garantit que le résultat est cohérent et constructible. Les outils IA dans ton plan (comme Lovable, Bolt.new ou Cursor) sont ceux que Gemini identifie comme options leaders selon ses données d'entraînement.",
  ...
  hi: {
    ...
    "faq.q5":             "Vibemaster अपने tech plans कैसे बनाता है?",
    "faq.a5":             "Vibemaster के tech plans <a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>, Google के large language model द्वारा generate किए जाते हैं। Gemini को सार्वजनिक डेटा के व्यापक संग्रह पर train किया गया है — documentation, developer forums, open-source repositories और technical articles — और वह इस ज्ञान का उपयोग tools, architectures और patterns की सिफारिश करने के लिए करता है। Vibemaster स्वतंत्र रूप से आपके plan में tools का चयन या ranking नहीं करता। इसके बजाय, हमारी proprietary prompt architecture Gemini से पूछे जाने वाले प्रश्नों को structure करती है। आपके plan में दिखने वाले AI tools (जैसे Lovable, Bolt.new या Cursor) वे हैं जिन्हें Gemini अपने training data के आधार पर leading options के रूप में identify करता है।",
  ...
  zh: {
    ...
    "faq.q5":             "Vibemaster如何生成技术方案？",
    "faq.a5":             "Vibemaster的技术方案由<a href="https://aistudio.google.com/" target="_blank" rel="noopener" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 transition-colors">Google Gemini</a>（谷歌的大型语言模型）生成。Gemini基于大量公开数据训练——包括文档、开发者论坛、开源代码库和技术文章——并利用这些知识推荐工具、架构和模式。Vibemaster不会独立选择或排名方案中的工具。相反，我们的专有提示架构构建我们向Gemini提出的问题，确保输出一致且可构建。方案中出现的AI工具（如Lovable、Bolt.new或Cursor）是Gemini根据其训练数据识别为领先选项的工具——我们还会额外验证这些工具与我们生成的输出配合效果，再向用户展示。",
